...
void setup() {
  ...
  M5.Lcd.setTextColor(RED, BLACK);// Sets the foreground color and background color of the displayed text.
...
void loop() {
  ...
  M5.Lcd.setCursor(
    0, 20);                               // Move the cursor position to (x,y). (x,y)
 ...
 //Make sure max is really max
  ...
  // get X, Y, Z Squared accX * accX = sq(accX)
    xSq = sq(accX);
    ySq = sq(accY);
    zSq = sq(accZ);

    tmpVec = sqrt(xSq + ySq + zSq);
    //   the tmp value is greater than maxVec than make them equal to each other. 
    if (tmpVec > maxVec){
        maxVec = tmpVec;
    }
